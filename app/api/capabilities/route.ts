...
export async function GET() {
  ...
  if (!token) return Response.json({ dlp: { available: false, protocol: null, qualities: [], codecs: [], containers: [] } }, { status: 401 });
  try {
    ...
    return Response.json({ dlp: { available: false, protocol: null, qualities: [], codecs: [], containers: [] } }, { status: 503 });
  }
}
